...
**訳文工房 (Yakubun Kobo)** は、Googleの強力なAIモデル「Gemini」を搭載した、Windows向けの多機能翻訳アプリケーションです。PDFやWordファイルの**AIによる高度な解析**、長文を快適に扱うための**ページング機能**、専門用語を正確に反映させる**ユーザー辞書**、訳文に解説を自動で加える**注釈機能**など、高度な機能で翻訳作業を強力にサポートします。
...
*   **AIファイル解析**: PDFファイル (`.pdf`)、Word文書 (`.docx`)、OpenDocumentテキスト (`.odt`) の読み込み時に、Gemini AIによる高度なファイル解析を利用できます（オプション設定で有効化）。複雑なレイアウトや複数カラムの文書も、AIが文書構造を理解して正確にテキストを抽出します。進捗は4段階（準備中→ファイル読み込み中→AI解析中→最終処理中）で表示され、処理状況を把握できます。
...
*   **テキストファイル**: `.txt` ファイルをドラッグ&ドロップするか、メニューバーの `ファイル` > `テキストファイルから読み込む` で読み込めます。
*   **AIファイル解析**: PDF、Word、OpenDocumentファイルの読み込みには、高度なAI解析機能を利用できます。この機能を有効にするには、メニューバーの `ツール` > `オプション` から「AIによるファイル解析を有効にする」をオンにしてください。
...
*   **機密情報とプライバシーについて**:
    ...
    *   **AIファイル解析機能**を有効にした場合、PDFやWord文書などのファイル内容もGemini APIに送信されます。この機能はデフォルトで無効になっており、オプション設定で明示的に有効化する必要があります。
